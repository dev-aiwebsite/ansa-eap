...
export default function HealthNewsCarousel({ className }: { className?: string }) {
  ...
  return (
    <div>
      <Carousel setApi={setApi} className={cn("w-full post-carousel", className)}>
        <CarouselContent>
          {healthNews.length > 0 ? healthNews.map((post, index) => (
          ...
          : 
            Array.from({ length: count }).map((_, index) => {
                return <CarouselItem key={index} className="basis-[80%]">
